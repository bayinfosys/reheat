...
from reheat.state import (ENRICHMENTS_TABLE, MODELS_TABLE, PROJECTIONS_TABLE,
                          REPORTS_TABLE, ClusterModel, CoverageData,
                          Enrichment, ModelClusterMetric,
                          ProjectionData, RunModels, ScatterData, SummaryData,
                          get_user_id)
...
def _enrichment_summary(e: Enrichment) -> dict:
    ...
    if e.enrichment_type == "adjacent":
        ...
        enriched = sum(1 for v in queries.values() if v.get("related"))
...
def cmd_runs_status(
    ...
    *,
    ...
    models = []
    for link in sorted(run_model_links, key=lambda link: link.model_id):
        model = backend.get(
            MODELS_TABLE,
            ClusterModel,
            user_id=get_user_id(backend),
